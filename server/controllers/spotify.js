...
exports.getNewTracks = async (req, res) => {
  ...
  responseArtist.albums.items.forEach((item) =>
    artists.push([
      item.artists[0].id,
      item.available_markets.includes("IN") ? "IN" : "US",
    ])
  );
  artists.forEach(async (artist, ind) => {
    let responseTrack = await fetch(
      `https://api.spotify.com/v1/artists/${artist[0]}/top-tracks?country=${artist[1]}`,
      {
        method: "GET",
        headers: {
          Accept: "application/json",
      ...
      response.json().then((data) => {
        if (data && data.tracks) {
          ...
          data.tracks.items.forEach((item, index) => {
            ...
            if (index == l - 1) res.status(200).json(results);
          });
        } else {
